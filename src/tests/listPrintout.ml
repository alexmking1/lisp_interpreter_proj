...
(* LOOKUP. Return the value of a symbol whose string is NAME. First search the
  ...
  let lookup env name =
    envGet env name
      (fun () -> envGet (! global) name (fun () -> oops ("Unbound name " ^ name))) ;;

(* EVALUATING. Do all the work for EVALUATE. *)
  let rec evaluating thing env =
    match thing with
      |Cons (func, args) -> (match (evaluating func env) with 
  ...
  and apply pars args argsEnv body bodyEnv =
    let rec applying pars args bodyEnv =
      match (pars, args) with 
        ...
        |(Cons (Symbol name, pars), Cons (arg, args)) -> applying pars args(envPut name (evaluating arg argsEnv) bodyEnv)
...
(* IS MEMBER. Test if THING is a member of the Lisp list THINGS. It's a helper
  ...
  let rec isMember thing things =
    match things with 
      | Cons (first, rest) -> thing = first || isMember thing rest
      |_ -> false ;;
  ...
  let rec areParameters things =
    match things with 
      ...
      | Cons (first, rest) -> (match first with 
                                | Symbol _ -> not (isMember first rest) && areParameters rest 
                                |_ -> false)
      |_ -> false ;;
...
(* MAKE ARITHMETIC. Return a HOW TO function that takes two NUMBER arguments,
  ...
  let makeArithmetic op message =
    (fun args env -> match args with 
        | Cons (left, Cons (right, Nil)) -> (let left = evaluating left env
...
(* -. Negate a single NUMBER argument, or subtract two NUMBER arguments. Return
  ...
  primitive "-"
    (fun args env ->
      match args with 
        |Cons (right, Nil) -> (match (evaluating right env) with 
                                |Number right -> Number (- right)
                                |_ -> oops "- expected 1 or 2 NUMBERs") 
        | Cons (left, Cons (right, Nil)) -> let left = evaluating left env
                                          in let right = evaluating right env
                                          in (match (left, right) with 
  ...
  primitive "/"
    (fun args env -> match args with 
          |Cons (left, Cons (right, Nil)) -> let left = evaluating left env
             in let right = evaluating right env
                in (match (left, right) with 
                |(Number 0, _) -> oops "/ tried to divide by 0" 
                |(Number left, Number right) -> Number (left / right)
                |_ -> oops "/ expected 2 NUMBERs")
          |_ -> oops "/ expected 2 NUMBERs") ;;
             ...
             in let right = evaluating right env
                in (match (left, right) with 
                  ...
                  |(Number _, Number _) | (Symbol _, Symbol _) ->
                         ...
                         (_, _) -> oops "= expected 2 ATOMs")
           |_ -> oops "= expected 2 ATOMs") ;;
  ...
  primitive "and"
    (fun args env ->
      let rec anding args =
        ...
        |_ ->oops "AND expected 0 or more THINGs"
      in anding args) ;;
...
(* ATOM. Test if the single argument is a NUMBER or a SYMBOL, returning either
  ...
  primitive "atom"
    (fun args env -> match args with 
      |Cons (arg, Nil) -> (match (evaluating arg env) with 
              |Number _ | Symbol _ -> tee 
              |_ -> Nil)
      |_ -> oops "ATOM expected 1 THING") ;;
...
(* CONS. Return a new list, whose first element is the first argument and whose
  ...
  primitive "cons"
    (fun args env ->match args with 
      |Cons (first, Cons (rest, Nil)) -> let first = evaluating first env
                              in let rest = evaluating rest env
                              in (match rest with 
                                  |Nil | Cons (_, _) -> Cons (first, rest) 
                                  |_ -> oops "CONS expected a THING and a LIST") 
      |_ -> oops "CONS expected a THING and a LIST") ;;
...
(* DEFINE. Take 2 arguments: a SYMBOL and a THING. Bind the unevaluated SYMBOL
  ...
  primitive "define"
    (fun args env -> match args with 
      |Cons (left, Cons (right, Nil)) -> (match left with 
              |Symbol name -> global := envPut name (evaluating right env) (! global) ; left
              |_ -> oops "DEFINE expected a SYMBOL and a THING") 
      |_ -> oops "DEFINE expected a SYMBOL and a THING") ;;
    ...
    (fun args env ->
      let rec oring args =
        match args with 
          |Nil -> Nil 
          |Cons (arg, Nil) -> evaluating arg env
          |Cons (arg, args) -> let value = evaluating arg env
                    in if value = Nil
                    then oring args
                    else value 
          |_ -> oops "OR expected 0 or more THINGs"
      in oring args) ;;
...
(* LAMBDA, λ. Return a closure. Its parameter list is the unevaluated first
  ...
  let howToLambda args env =
    ...
    |_ -> oops "LAMBDA or λ expected parameters and a body" ;;
  ...
  primitive "list"
    (fun args env ->
      let rec listing args =
        match args with 
        |Nil -> Nil 
        |Cons (arg, args) -> Cons (evaluating arg env, listing args) 
        |_ ->oops "LIST expected 0 or more THINGs"
      in listing args) ;;
