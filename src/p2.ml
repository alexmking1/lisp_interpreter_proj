...
struct
    ...
    let makeParser path = 
      let nextToken = Scanner.makeScanner path
        ...
        let rec nextThing () = 
            match !token with
              |Scanner.CloseParenToken -> raise (CantParse "Error: No OpenParenToken found")
              |Scanner.EndToken -> raise (CantParse "Error: EndToken not expected")
              |Scanner.NumberToken number -> token := nextToken () ; Number number
              |Scanner.OpenParenToken -> token := nextToken () ; nextThings ()
              |Scanner.SymbolToken "nil" ->  token := nextToken () ; Nil
              |Scanner.SymbolToken name -> token := nextToken () ; Symbol name
              
          and nextThings () =
            match !token with
              |Scanner.CloseParenToken -> token := nextToken () ; Nil
              |Scanner.EndToken -> raise (CantParse "Error: list ended without a CloseParenToken")
              |_-> let left = nextThing ()
...
(*MY TEST RESULTS RUNNING FROM TERMINAL:
   ...
   (Cons (Symbol "lambda",
     Cons (Cons (Symbol "n", Nil),
      Cons
       ...
       Nil))),
   Nil)))
Exception: Parser.CantParse "Error: EndToken not expected".
